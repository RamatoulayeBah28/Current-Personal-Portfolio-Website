...
function filterProjects(c) {
  var x, i;
  x = document.getElementsByClassName("details-container color-container");
  if (c == "all") c = "";
  // Add the "show" class (display:block) to the filtered elements, and remove the "show" class from the elements that are not selected
  for (i = 0; i < x.length; i++) {
	removeClass(x[i], "show");
	console.log(`Checking element ${i} with class: ${x[i].className}`);
	if (x[i].className.indexOf(c) > -1) {
	  addClass(x[i], "show");
	  console.log(`Added show to element ${i}`);
	}
  }  
}
...
function viewDetails(projectId) {
  ...
  const content = document.querySelector(`#${projectId}`);
  modal.innerHTML = content.innerHTML; // Or populate dynamically
